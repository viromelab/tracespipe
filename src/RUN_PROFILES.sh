...
# |     =================================================================      |
# |                                                                            |
# |                ./Run_profile.sh ids.txt VDB.fa reads.fq blood              |
# |                                                                            |
...
RUN_COMPARISON=0;
#
RUN_RELATIVE=0; # reads.fq ARE NEEDED FOR THIS COMPUTATION
#
# ==============================================================================
# ================================== GET GTO ===================================
if [[ "$GET_GTO" -eq "1" ]];
...
if [[ "$RUN_COMPARISON" -eq "1" ]];
  ...
  for ids_gen in ${IDS_ARRAY[@]};
    ...
    gnuplot << EOF
      ...
      set output "$4-profile_self-$ids_gen.pdf"
...
EOF
    if [[ "$RUN_RELATIVE" -eq "1" ]];  # HERE reads.fq WILL BE NEEDED!
      ...
      gnuplot << EOF
        reset
        set terminal pdfcairo enhanced color font 'Verdana,12'
        set output "$4-profile_relative-$ids_gen.pdf"
        set style line 101 lc rgb '#000000' lt 1 lw 4
        set border 3 front ls 101
        set tics nomirror out scale 0.75
        set format '%g'
...
EOF
      ...
      gnuplot << EOF
        reset
        set terminal pdfcairo enhanced color font 'Verdana,12'
        set output "$4-profiles-$ids_gen.pdf"
        set style line 101 lc rgb '#000000' lt 1 lw 4
        set border 3 front ls 101
        set tics nomirror out scale 0.75
        set format '%g'
